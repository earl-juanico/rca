...
class CustomMoondreamModel(MoondreamModel):
    def _decode_one_tok(self, x, attn_mask, pos_ids):
        hidden = text_decoder(x, self.text, attn_mask, pos_ids, self.config.text)
        # --- MODIFY hidden here ---
        hidden = self.modify_hidden_states(hidden)
        logits = lm_head(hidden, self.text)
        return logits, hidden
    
    def modify_hidden_states(self, hidden_states):
        # Example modification: apply a threshold and ReLU
        threshold = -1.5 #-1.5  # z >= c, wherein c = threshold
